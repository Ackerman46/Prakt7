...
namespace WpfApp1
{
    ...
    public partial class MainWindow : Window
    {
        ...
        vector3D vectors3D1 = new vector3D();
        vector3D vectors3D2 = new vector3D();
        public MainWindow()
        {
            InitializeComponent();
        }
        ...
        {
            MessageBox.Show("Разработчик: Агеев Алексей\nЗадание: Использовать класс Triad (тройка чисел). Создать производный класс vector3D, задаваемый тройкой координат. \nДолжны быть реализованы: операция сложения векторов, скалярное произведение векторов.", "Вариант 12", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        ...
        private void vectorsSums_Click(object sender, RoutedEventArgs e)
        {
            GetVectorsValue();
            MessageBox.Show($"Сумма первого вектора: ({vectors3D1.A} + {vectors3D1.B} + {vectors3D1.C}) = {vectors3D1.vectorsSum()} \nСумма второго вектора: ({vectors3D2.A} + {vectors3D2.B} + {vectors3D2.C}) = {vectors3D2.vectorsSum()}");
        }
        private void GetVectorsValue()
        {
            int.TryParse(valueOneTriad1.Text, out int valOneTriad1);
            int.TryParse(valueTwoTriad1.Text, out int valTwoTriad1);
            int.TryParse(valueThreeTriad1.Text, out int valThreeTriad1);

            vectors3D1.A = valOneTriad1;
            vectors3D1.B = valTwoTriad1;
            vectors3D1.C = valThreeTriad1;

            int.TryParse(valueOneTriad2.Text, out int valOneTriad2);
            int.TryParse(valueTwoTriad2.Text, out int valTwoTriad2);
            int.TryParse(valueThreeTriad2.Text, out int valThreeTriad2);

            vectors3D2.One = valOneTriad2;
            vectors3D2.Two = valTwoTriad2;
            vectors3D2.Three = valThreeTriad2;
        }
    }
}
